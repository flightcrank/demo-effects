...
int main (int argc, char* args[]) {
	...
	char msg[] = "this is a scroller that displays a string of text and places a variable number of diffrent segments of each individual char at a diffrent y positions along a sine wave";
	...
	while(quit == 0) {
		...
		for (i = 0; i < strlen(msg); i++) {
			...
			int slice_num = 8; //number of horizontal slices to take of each char
			float slice_size = CHAR_WIDTH / 8; //number of horizontal image pixel slices to take for each char (should even dvide into CHAR_WIDTH)

			int j;

			for (j = 0; j < slice_num; j++) {
				
				src.x = v.x + j * slice_size;
				src.y = v.y;
				src.w = slice_size;
				src.h = CHAR_WIDTH;
